...
| UPG-03 | integrated | CLI / JSON Server / TTY → `runAgentTurn` → `RunBudget` → model/tool execution | `src/run-budget.ts`、`src/agent-loop.ts`、`src/tool.ts`、三个主入口及预算/错误契约测试 | `npm.cmd run check`、`npm.cmd run lint` 通过；`npm.cmd test`：218/218 | 已集中模型/工具/Token/墙钟/返工/深度/并发预算，连续重复结果硬终止，并归一化 invalid_arguments/forbidden/transient/timeout/cancelled/version_mismatch/internal。工具仍串行，主动取消、执行中超时中断和自动有限退避尚未完成 | 补齐工具超时与有限重试，然后进入 UPG-04 真流式和取消贯通 |
