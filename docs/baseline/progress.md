...
| UPG-03 | integrated | CLI / JSON Server / TTY → `runAgentTurn` → `RunBudget` → model/tool execution | `src/run-budget.ts`、`src/agent-loop.ts`、三个主入口及预算测试 | `npm.cmd run check`、`npm.cmd run lint` 通过；`npm.cmd test`：214/214 | 已集中模型/工具/Token/墙钟/返工/深度/并发预算，连续重复结果硬终止；Token 缺失时保守估算。工具仍串行，主动取消、超时中断、结构化错误归一化尚未完成 | 补齐工具错误契约与有限重试，然后进入 UPG-04 真流式和取消贯通 |
