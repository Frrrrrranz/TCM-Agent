...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/测试-252%20passed-4CAF50?style=for-the-badge" alt="测试: 252 passed" />
</p>
...
**TCM-Agent** 是一个面向中医药场景的 AI Agent 问答系统，集成 MCP 工具调用、RAG 检索、结构化输出与 Web 交互能力。项目基于 [MiniCode](https://github.com/LiuMengxuan04/MiniCode) 框架，通过注入中医角色知识与规则，结合 Python MCP Server 提供的数据引擎，实现中药查询、配伍禁忌检查、辨证分析等核心能力。

### 核心成果

| 成果 | 说明 |
|------|------|
| **三层解耦架构** | Agent 框架层、Markdown 知识层、Python MCP 数据引擎通过 MCP 协议通信，领域知识与底层框架彻底解耦。 |
| **MCP 工具设计** | 6 个结构化工具，覆盖中药查询、方剂检索、辨证分析、穴位查询、医案检索、配伍禁忌检查；统一参数 Schema 与返回格式。 |
| **混合检索链路** | SQLite 精确查询 + ChromaDB 语义检索 (12万+ 记录) + 重排序评测；小规模评测 Recall@5 达 1.0。 |
| **会话记忆系统** | 异步 SQLite 持久化 (aiosqlite)，支持多轮记忆摘要注入、Fail-Open 容错，覆盖 28 万+ 入库流水。 |
| **安全规则引擎** | 内置十八反 / 十九畏及别名映射，配伍禁忌、毒性药物、妊娠禁忌全覆盖，零漏报。 |
| **评测与 CI** | Pytest Harness + GitHub Actions CI，Agent 204 测试 + MCP 48 测试全部通过。 |
| **Web 交互原型** | Vue 3 + FastAPI Web Gateway，支持 WebSocket 流式对话、工具调用状态展示、辨证结果结构化渲染。 |
...
| [CLAUDE_CODE_PATTERNS.md](./CLAUDE_CODE_PATTERNS.md) | MiniCode 与 Claude Code 架构与设计模式对比说明 |
